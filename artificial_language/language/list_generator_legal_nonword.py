...
if __name__ == '__main__':
    language = 'datives'
    nonword_path = Path.cwd().joinpath(f'{language}/lexicon/representations/phonology_artificial_legal_nonwords.csv')
    ...
    list_length = 1
    num_lists = 500
    all_lists = generate_n_random_lists(n = num_lists, items = nonwords, list_length = list_length, seed = seed)
    all_lists = pd.DataFrame(all_lists)

    # Save memory lists
    memory_list_path = Path.cwd().joinpath(f'language/{language}/nonword_memory_lists_legal_length{list_length}_n{num_lists}.tsv')
