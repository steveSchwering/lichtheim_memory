...
def save_random_lists(random_lists, language_path,
                      utterance_header = ['utterance_str', 'utterance_lst'],
                      random_list_filename = 'random_lists.tsv',
                      sep = '\t'):
    """
    """
...
if __name__ == '__main__':
    # Get sentences
    language = 'datives'
    language_path = Path.cwd().joinpath(f'language/{language}')
    ...
    random_lists = generate_combinations_and_check_bigrams(sentences = sentences, bigrams = bigrams)
    save_random_lists(random_lists = random_lists, language_path = language_path)